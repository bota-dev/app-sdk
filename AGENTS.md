...
- `frameworks/web` is the foreground browser facade and publishes as
  ...
  `1.2.0-beta.9`, `1.2.0-beta.10`, `1.2.0-beta.11`, and `1.2.0-beta.12` have
...
- Prepared `1.2.0-beta.0` metadata and evidence do not claim publication. That
  ...
  `prepare_command`, which Trunk rejects for a new pod. `v1.2.0-beta.8` is
  immutable and partially published: its Apple SwiftPM, CocoaPods, Android,
  React Native, and Web artifacts are public, but the pod workflow failed after
  ...
  CocoaPods 1.16.2 executable. `v1.2.0-beta.11` is also immutable and
  partially published: its native, npm, and CocoaPods packages are public,
  but the Flutter job selected a different Ruby/CocoaPods installation and
  failed its exact-version gate. `1.2.0-beta.12` is the
