...
function u64LeHex(bytes: Uint8Array): string {
...
function decodeDetail(bytes: Buffer): Pick<DeviceDiagnosticEvent,
  ...
  const firmwareBuildId = Buffer.from(bytes.subarray(2, 14)).toString('ascii');
  ...
  const task = Buffer.from(bytes.subarray(80, 96)).toString('ascii').replace(/\0.*$/, '');
  const breadcrumbs = [];
