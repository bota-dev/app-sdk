...
func awaitWorkflowCompletion(_ command: CoreCommand, runtime: DeviceRuntime) async throws {
    ...
    } catch let error as BotaDeviceSDKError {
        throw error
    } catch let error as CoreError {
        throw BotaDeviceSDKError(error)
    }
    guard completed else {
        throw BotaDeviceSDKError(
            code: .internal,
            operation: BotaDeviceSDKError.operation(command.kind),
            retryable: true,
...
func workflowError(_ notification: CoreNotification) -> BotaDeviceSDKError {
    ...
    return BotaDeviceSDKError(
        code: BotaDeviceSDKError.code(UInt32(clamping: rawCode)),
        operation: BotaDeviceSDKError.operation(notification.packet.operation),
