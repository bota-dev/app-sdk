...
- [ ] **Step 3: Implement the Swift plugin and adapter**
  ...
  Direct CoreBluetooth reads and writes install cancellation-aware gate waiters and continuations before dispatching native work. Task cancellation removes and resumes the matching waiter or continuation exactly once; registration races and late delegate callbacks cannot revive it. A cancelled read characteristic rejects reuse until an unambiguous stale response or disconnect clears quarantine; same-characteristic notifications continue to their subscriber and never clear that boundary. Subscription startup is tracked before awaiting native stream creation. Detach and public cancellation retain process-wide ownership, cancel and await that exact start task, and perform native stop after startup unwinds so a pre-start no-op is not terminal proof. The coordinator retains a failed-cancellation owner until the original operation terminally unwinds or final shared-client destruction proves native cleanup; a terminal stream collector is not such proof after stop throws. Another engine cannot acquire or cancel that category during recovery.
