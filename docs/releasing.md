...
its package bytes matched the main-CI inventory. `1.2.0-beta.10` is the selected
...
release dependency graph. Do not move or reuse beta.9. Beta.10 uses explicit
status conditions at all three Flutter-side jobs and still requires each direct
prerequisite to succeed.
Apple consumers add
...
`1.2.0-beta.10`; do not move, delete, recreate, or reuse `v1.2.0-beta.0`
through `v1.2.0-beta.9`.
...
`1.2.0-beta.8`, `1.2.0-beta.9`, and `1.2.0-beta.10`, the release owner explicitly requested a public beta rollout
...
[`release/evidence/1.2.0-beta.10-web-foreground.md`](../release/evidence/1.2.0-beta.10-web-foreground.md).
...
The first Flutter beta may be published only after every `1.2.0-beta.10` version
...
`1.2.0-beta.0`. For selected `1.2.0-beta.10`, it writes
...
10. Refuses the occupied `1.2.0-beta.0` identity. For selected
    `1.2.0-beta.10`, its explicitly authorized first-publish
