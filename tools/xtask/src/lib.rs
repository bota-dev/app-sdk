...
pub mod release {
    ...
    pub fn generate_client_identity(root: &Path, check: bool) -> Result<(), String> {
        let version: SdkVersion = parse_toml_file(&root.join("sdk-version.toml"))?;
        let header = "// Generated by cargo xtask client-identity generate. Do not edit.\n";
        for (platform, relative) in [
            ("web", "frameworks/web/src/sdkIdentity.ts"),
            ("react-native", "frameworks/react-native/src/sdkIdentity.ts"),
            (
                "apple",
                "platforms/apple/Sources/BotaAppSDK/SdkIdentity.swift",
            ),
            (
                "android",
                "platforms/android/sdk/src/main/kotlin/dev/bota/sdk/SdkIdentity.kt",
            ),
            (
                "flutter",
                "frameworks/flutter/bota_app_sdk/lib/src/sdk_identity.dart",
            ),
        ] {
            let package = public_package(platform, &version.version)?;
            let sdk_version = &version.version;
            let body = match platform {
                "apple" => format!(
                    "enum SdkIdentity {{\n    static let package = \"{package}\"\n    static let version = \"{sdk_version}\"\n}}\n"
                ),
                "android" => format!(
                    "package dev.bota.sdk\n\ninternal object SdkIdentity {{\n    const val PACKAGE = \"{package}\"\n    const val VERSION = \"{sdk_version}\"\n}}\n"
                ),
                "flutter" => format!(
                    "const sdkPackage = '{package}';\nconst sdkVersion = '{sdk_version}';\n"
                ),
                _ => format!(
                    "export const SDK_PACKAGE = '{package}'\nexport const SDK_VERSION = '{sdk_version}'\n"
                ),
            };
            ...
            if check {
                return Err(format!(
                    "stale SDK identity: {relative}; run cargo xtask client-identity generate"
                ));
            }
...
pub fn run(args: impl IntoIterator<Item = OsString>) -> Result<(), String> {
    ...
    match args.as_slice() {
        [identity, generate] if identity == "client-identity" && generate == "generate" => {
            let root = std::env::current_dir().map_err(|error| error.to_string())?;
            release::generate_client_identity(&root, false)
        }
        [identity, generate, check] if identity == "client-identity" && generate == "generate" && check == "--check" => {
            let root = std::env::current_dir().map_err(|error| error.to_string())?;
            release::generate_client_identity(&root, true)
        }
        ...
        _ => Err(
            "usage: cargo xtask <client-identity generate [--check] | protocol generate [--check] | encrypted-upload-v2 vectors generate [--check] | release validate <manifest.json> | release verify-tag <vVERSION>>".to_owned(),
        ),
    }
}
