...
fn release_metadata_fixture() -> PathBuf {
    ...
    for path in [
        ...
        "release/examples/1.2.0-beta.6.json",
    ] {
...
fn version_tag_and_publishable_metadata_are_synchronized() {
    let release = xtask::release::verify_release(&root(), "v1.2.0-beta.6").unwrap();

    assert_eq!(release.version, "1.2.0-beta.6");
...
fn every_public_package_version_copy_fails_closed_on_drift() {
    for (path, needle, replacement) in [
        ...
        let error = xtask::release::verify_release(&fixture, "v1.2.0-beta.6").unwrap_err();
        assert!(
...
fn mismatched_or_unprefixed_tags_are_rejected() {
    ...
    let missing_prefix = xtask::release::verify_release(&root(), "1.2.0-beta.6").unwrap_err();
