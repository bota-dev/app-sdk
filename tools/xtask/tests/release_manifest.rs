...
fn example() -> serde_json::Value {
    let contents = fs::read_to_string(root().join("release/examples/1.2.0-beta.10.json")).unwrap();
...
fn example_release_manifest_is_valid() {
    let manifest = root().join("release/examples/1.2.0-beta.10.json");
...
fn published_v1_manifest_remains_valid_independent_of_later_checkout_version() {
    ...
    assert!(
        current_release_result
            .unwrap_err()
            .contains("sdkVersion 1.0.0 does not match sdk-version.toml 1.2.0-beta.10")
    );
}
