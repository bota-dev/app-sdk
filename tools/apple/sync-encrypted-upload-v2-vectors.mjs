...
if (check) {
  ...
  if (JSON.stringify(names) !== JSON.stringify(destinationNames)) {
    throw new Error("Apple encrypted-upload-v2 resource list is stale");
  }
  ...
  if (!bytes.equals(actualBytes) || !digestBytes.equals(actualDigest)) {
    throw new Error("Apple encrypted-upload-v2 resources are stale");
  }
  console.log(`Apple encrypted-upload-v2 vectors are current (${digest})`);
} else {
  ...
  console.log(`Synced Apple encrypted-upload-v2 vectors (${digest})`);
}
