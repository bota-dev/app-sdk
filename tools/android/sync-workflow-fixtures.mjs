...
const destination = path.join(
  root,
  "platforms/android/sdk/src/androidTest/assets/WorkflowFixtures/workflows.json",
);
...
if (check) {
  ...
  if (actual !== generated) throw new Error("Android workflow fixture resource is stale");
  console.log(`Android workflow fixtures are current (${scenarios.length} scenarios)`);
} else {
  await mkdir(path.dirname(destination), { recursive: true });
  await writeFile(destination, generated);
  console.log(`Synced ${scenarios.length} Android workflow scenarios`);
}
