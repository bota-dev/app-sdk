...
const destination = path.join(
  root,
  "platforms/android/sdk/src/androidTest/assets/ProtocolFixtures",
);
...
if (check) {
  ...
  if (JSON.stringify(names) !== JSON.stringify(destinationNames)) {
    throw new Error("Android protocol fixture resource list is stale");
  }
  for (const name of names) {
    ...
    if (!expected.equals(actual)) {
      throw new Error(`Android protocol fixture resource is stale: ${name}`);
    }
  }
  console.log(`Android protocol fixtures are current (${names.length} suites)`);
} else {
  ...
  console.log(`Synced ${names.length} Android protocol fixture suites`);
}
