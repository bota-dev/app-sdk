...
readonly DESTINATION="$REPOSITORY/dev/bota/bota-app-sdk/$VERSION"

"$ROOT/tools/android/verify-publication.sh" "$RELEASE_DIRECTORY"
rm -rf "$REPOSITORY"
mkdir -p "$DESTINATION"
cp "$RELEASE_DIRECTORY"/bota-app-sdk-"$VERSION"* "$DESTINATION/"
test -s "$DESTINATION/bota-app-sdk-$VERSION.aar"
