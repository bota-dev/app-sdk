...
validates a low-volume lifecycle, device-connection, device-status, and
nonce-bound provisioning TurboModule contract for iOS and Android, and
...
reads. Its one-shot material broker delegates provisioning and remove-only
deprovision without reopening the nonce race. A disposable CocoaPods
...
status, and provisioning slice through `BotaDeviceClient.shared`; a
