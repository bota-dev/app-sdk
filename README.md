...
`1.2.0-beta.9` candidate. Its supervised physical Chromium matrix has not run;
...
`1.2.0-beta.9` is the selected synchronized replacement candidate. Automatic
...
The next synchronized version is `1.2.0-beta.9`. The prepared source composes
...
and held Flutter. Beta.9 checks Trunk directly before any push.
...
`1.2.0-beta.9` is published and verified on pub.dev.
...
because that immutable tag identifies non-Flutter source. `1.2.0-beta.9` is the
...
The published beta.8 remains available for supervised testing. After beta.9
completes, install its exact version:

```bash
npm install @bota.dev/web-sdk@1.2.0-beta.9
```
...
| Capability | `1.2.0-beta.9` candidate |
|---|---|
