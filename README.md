...
`1.2.0-beta.3` candidate. Its supervised physical Chromium matrix has not run;
...
`1.2.0-beta.3` is the selected synchronized replacement candidate. Automatic
...
The next synchronized version is `1.2.0-beta.3`. The prepared source composes
...
`1.2.0-beta.3` is published and verified on pub.dev.
...
because that immutable tag identifies non-Flutter source. `1.2.0-beta.3` is the
...
After the `1.2.0-beta.3` candidate is published, install the exact beta:

```bash
npm install @bota.dev/web-sdk@1.2.0-beta.3
```
...
| Capability | `1.2.0-beta.3` candidate |
|---|---|
